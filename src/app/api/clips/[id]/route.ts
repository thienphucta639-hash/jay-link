...
export async function GET(_req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const tagRows = await db.select({ tagName: tags.name, tagId: tags.id }).from(clipTags)
      .innerJoin(tags, eq(tags.id, clipTags.tagId)).where(eq(clipTags.clipId, clipId));
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    for (const f of ["customTitle","note","saveReason","watchStatus","isPinned","collectionId","creatorName","creatorHandle","previewImage"]) {
      if (body[f] !== undefined) u[f] = body[f];
    }
